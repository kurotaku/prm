module MakerPage
  class FilesController < MakerPageController
    # include DownloadFile

    def index
      @files = @current_maker_group.shared_files.page(params[:page]).per(10)
    end
    ...
    def create
      maker_group = MakerGroup.find_by(uid: params[:base_path])
      @file = current_user.shared_files.build(file_params.merge(maker_group_id: maker_group.id))
      if @file.save
        flash['success'] = t('shared_files.create.success')
        redirect_to maker_page_files_path
      else
